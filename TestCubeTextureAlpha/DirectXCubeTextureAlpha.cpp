...
CDirectXCubeTextureAlpha::~CDirectXCubeTextureAlpha()
{
}

/**
 * @ingroup TestCubeTextureAlpha
 * @brief 삼각형을 그리기 위한 준비 작업을 수행한다.
 * @returns 성공하면 true 를 리턴하고 그렇지 않으면 false 를 리턴한다.
 */
bool CDirectXCubeTextureAlpha::CreateChild()
{
	// 큐브 정점 정보
	Vertex arrCube[] =
	{
	...
	// 텍스처 이미지 파일을 로드한다.
	CHECK_FAILED( D3DX11CreateShaderResourceViewFromFile( m_pclsDevice, L"Texture/WireFence.dds", 0, 0, &m_pclsShaderResView, 0 ) );

	// 앞면의 철망을 통해서 반대편 면의 철망도 보이게 하기 위해서 후면 선별을 비활성하기 위한 ID3D11RasterizerState 를 생성한다.
	D3D11_RASTERIZER_DESC sttRD;

	memset( &sttRD, 0, sizeof(sttRD) );
	sttRD.FillMode = D3D11_FILL_SOLID;
	sttRD.CullMode = D3D11_CULL_NONE;
	sttRD.FrontCounterClockwise = false;
	sttRD.DepthClipEnable = true;
	CHECK_FAILED( m_pclsDevice->CreateRasterizerState( &sttRD, &m_pclsRS ) );
 ...
 * @returns 성공하면 true 를 리턴하고 그렇지 않으면 false 를 리턴한다.
 */
bool CDirectXCubeTextureAlpha::DrawChild()
{
	...
	for( UINT p = 0; p < sttTechDesc.Passes; ++p )
	{
		...
		// 철망 사이로 삼각형 뒷면이 보여야 하므로 후면 선별을 적용하지 않는다.
		m_pclsContext->RSSetState( m_pclsRS );

		m_pclsEffectTech->GetPassByIndex(p)->Apply( 0, m_pclsContext );
		m_pclsContext->DrawIndexed( m_iIndexCount, 0, 0 );

		// 기본 렌더 상태를 복원한다.
		m_pclsContext->RSSetState( NULL );
	}
 ...
 * @returns true 를 리턴한다.
 */
bool CDirectXCubeTextureAlpha::Update()
{
 ...
 * @param y			마우스 Y 위치
 */
void CDirectXCubeTextureAlpha::OnMouseDown( HWND hWnd, int x, int y )
{
	m_sttMousePos.x = x;
	m_sttMousePos.y = y;
	m_bMouseDown = true;
 ...
 * @param y			마우스 Y 위치
 */
void CDirectXCubeTextureAlpha::OnMouseUp( int x, int y )
{
	ReleaseCapture();
	m_bMouseDown = false;
}
 ...
 * @param y			마우스 Y 위치
 */
void CDirectXCubeTextureAlpha::OnMouseMove( HWND hWnd, int x, int y )
{
	if( m_bMouseDown )
	{
