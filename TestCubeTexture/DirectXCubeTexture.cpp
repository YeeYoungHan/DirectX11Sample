...
CDirectXCubeTexture::CDirectXCubeTexture() : m_bMouseDown(false), m_iIndexCount(0)
{
	memset( &m_sttMousePos, 0, sizeof(m_sttMousePos) );
}

CDirectXCubeTexture::~CDirectXCubeTexture()
{
}
 ...
 * @returns 성공하면 true 를 리턴하고 그렇지 않으면 false 를 리턴한다.
 */
bool CDirectXCubeTexture::DrawChild()
{
 ...
 * @returns true 를 리턴한다.
 */
bool CDirectXCubeTexture::Update()
{
 ...
 * @param y			마우스 Y 위치
 */
void CDirectXCubeTexture::OnMouseDown( HWND hWnd, int x, int y )
{
	m_sttMousePos.x = x;
	m_sttMousePos.y = y;
	m_bMouseDown = true;
 ...
 * @param y			마우스 Y 위치
 */
void CDirectXCubeTexture::OnMouseUp( int x, int y )
{
	ReleaseCapture();
	m_bMouseDown = false;
}
 ...
 * @param y			마우스 Y 위치
 */
void CDirectXCubeTexture::OnMouseMove( HWND hWnd, int x, int y )
{
	if( m_bMouseDown )
	{
